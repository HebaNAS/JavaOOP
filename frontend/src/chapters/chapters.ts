...
export const CHAPTERS: Chapter[] = [
  ...
  // ─── CHAPTER 4: Methods ───
  {
    ...
    description: 'Warriors need abilities! Write attack(Warrior target) so the target actually loses health, and defend() so it actually raises the shield. The game runs YOUR code — if your body doesn\'t reduce health, no damage. If defend() doesn\'t set this.shielded = true, no shield (and the enemy will hit you full force).',
    hints: [
      'Methods are the verbs of a class. The game runs WHAT YOU WRITE inside them. attack(target) must subtract from target.health; defend() takes NO parameters and must set this.shielded = true. Method NAME alone does nothing — the body is what happens.',
      'attack(Warrior target): "target.health = target.health - this.attackPower;" — the game reads target.health before AND after your method; the difference is the damage shown on screen. defend(): "this.shielded = true;" — the engine checks that flag on the next enemy hit and halves the damage. Don\'t put `this.health -= ...` in defend (that hurts you instead of protecting).',
      'If attack() is empty: swing but 0 damage. If defend() is empty (or touches health instead of shielded): no shield, full enemy hit. If movement bodies don\'t change this.x / this.y: the character stays put. Your code IS the behaviour.',
    ],
    starter: 'class Warrior {\n    String name;\n    int health;\n    int attackPower;\n    // Shield flag — defend() MUST set this to true for the shield to work.\n    // The game halves the next enemy hit only if this is true.\n    boolean shielded;\n    int x;   // column on the map\n    int y;   // row on the map\n\n    Warrior(String name, int health, int attackPower) {\n        this.name = name;\n        this.health = health;\n        this.attackPower = attackPower;\n    }\n\n    // attack(target): subtract YOUR attackPower from target.health.\n    // The game reads target.health before/after — the difference IS the\n    // damage shown on screen. Empty body = 0 damage = no enemy hurt.\n    public void attack(Warrior target) {\n        target.health = target.health - this.attackPower;\n    }\n\n    // defend(): NO parameters. Set this.shielded = true so the next enemy\n    // hit gets halved. Touching health here would HURT YOU, not protect.\n    public void defend() {\n        this.shielded = true;\n    }\n\n    // Movement — WASD / arrow keys call these at runtime.\n    // Whatever your body does to this.x / this.y IS what happens on screen.\n    public void moveUp()    { this.y -= 1; }\n    public void moveDown()  { this.y += 1; }\n    public void moveLeft()  { this.x -= 1; }\n    public void moveRight() { this.x += 1; }\n}\n\nWarrior hero  = new Warrior("Aldric", 100, 25);\nWarrior enemy = new Warrior("Grok",    80, 20);\n\n// Click an enemy (red ring appears) then SPACE = attack, Q = defend.\nhero.attack(enemy);\nhero.defend();\n',
    validate(p) {
      const c = p.classes.find((c) => c.name === 'Warrior')
      if (!c) return { pass: false, msg: 'Keep your Warrior class!' }
  ...
  // ─── CHAPTER 6: Inheritance ───
  {
    ...
    description: 'A Mage inherits everything from Warrior but adds mana and castSpell(). Create a Mage class that EXTENDS Warrior! castSpell must reduce target.health for damage to actually happen — the game runs your body, not your method name.',
    hints: [
      ...
      'castSpell(Warrior t): "t.health -= this.attackPower;" — same WYSIWYG rule as attack(). Empty body = the spell fizzles for 0 damage. (super() is later in chapter 16; for now Mage\'s constructor can mirror Warrior\'s.)',
    ],
    starter: 'class Warrior {\n    String name;\n    int health;\n    int attackPower;\n    boolean shielded;\n\n    Warrior(String name, int health, int attackPower) {\n        this.name = name;\n        this.health = health;\n        this.attackPower = attackPower;\n    }\n\n    public void attack(Warrior target) {\n        target.health = target.health - this.attackPower;\n    }\n    // defend(): no parameters. Set this.shielded = true to raise the shield.\n    public void defend() { this.shielded = true; }\n}\n\n// Create: class Mage extends Warrior\n//   Include: int mana\n//   castSpell(Warrior t) — body MUST do: t.health -= this.attackPower;\n//   Otherwise the spell visually fires but deals zero damage.\n\nWarrior hero = new Warrior("Aldric", 100, 25);\n// Create a Mage object too!\n',
    validate(p) {
  ...
  // ─── CHAPTER 7: Polymorphism ───
  {
    ...
    description: 'Override attack() in Mage so it behaves like an UNRELIABLE SPARK — about 70% of the time it lands and deals DOUBLE damage, the other 30% it fizzles (no damage). Same call (`x.attack(enemy)`) now does totally different things depending on whether x is a Warrior or a Mage. That\'s polymorphism. Press SPACE several times to feel the randomness.',
    hints: [
      'Polymorphism means: same method call, different behaviour, picked by the runtime class. Warriors swing reliable swords. Mages channel raw magic — devastating when it lands, but unpredictable. Override Mage.attack so it has that personality.',
      'Use Math.random() — it returns a double from 0.0 (inclusive) to 1.0 (exclusive). Compare it to a threshold like 0.7 with `<`. Inside the if-block, deal 2 × this.attackPower damage. Outside the if-block: do nothing — the body just exits, the snapshot/delta sees no health change, and the trace shows "swings — no damage".',
      'Inside Mage: public void attack(Warrior target) { if (Math.random() < 0.7) { target.health -= this.attackPower * 2; } } — that\'s the whole body. Then call hero.attack(enemy) AND wizard.attack(enemy). Click an enemy to lock target (red ring) and tap SPACE a few times: some hits deal 70 dmg, some deal 0.',
    ],
    starter: 'class Warrior {\n    String name;\n    int health;\n    int attackPower;\n    boolean shielded;\n\n    Warrior(String name, int health, int attackPower) {\n        this.name = name;\n        this.health = health;\n        this.attackPower = attackPower;\n    }\n\n    public void attack(Warrior target) {\n        target.health = target.health - this.attackPower;\n    }\n    public void defend() { this.shielded = true; }\n}\n\nclass Mage extends Warrior {\n    int mana;\n\n    // super(...) calls Warrior\'s constructor so name/health/attackPower\n    // get set the same way as a plain Warrior. (Chapter 16 explains why.)\n    Mage(String name, int health, int attackPower) {\n        super(name, health, attackPower);\n    }\n\n    // ─── YOUR JOB: override attack() ───\n    //\n    // The brief: an UNRELIABLE SPARK. About 70% of the time the magic\n    // connects and deals DOUBLE this.attackPower damage; the other 30%\n    // it fizzles and the target takes no damage at all.\n    //\n    // The tools you need:\n    //   • Math.random() — returns a double in [0.0, 1.0). Treat it as a\n    //     dice roll: < 0.7 means \"the spark connects\", anything else\n    //     means \"it fizzled\".\n    //   • An if-block — only run the damage line when the roll succeeds.\n    //   • this.attackPower — your stat, set by the constructor.\n    //   • target.health — what you subtract from when you connect.\n    //\n    // Skeleton (fill in the if-condition and the damage line):\n    //\n    //   public void attack(Warrior target) {\n    //       if ( /* roll < 0.7 */ ) {\n    //           // target.health -= ... (double the attackPower)\n    //       }\n    //       // no else needed — silence IS the fizzle\n    //   }\n    //\n    // After you Run, click the enemy to lock it (red ring) and tap SPACE\n    // a few times. You\'ll see some hits land for 70, others show 0.\n\n    // castSpell is Mage-only and stays reliable — always lands.\n    // (Same WYSIWYG rule: the body decides the damage.)\n    public void castSpell(Warrior target) {\n        target.health = target.health - this.attackPower;\n    }\n}\n\nWarrior hero = new Warrior("Aldric", 100, 25);\nMage wizard  = new Mage("Elara", 80, 35);\nWarrior enemy = new Warrior("Grok", 120, 20);\n\n// Call attack() on BOTH hero and wizard. Hero hits reliably for 25;\n// wizard\'s spark either lands for 70 or fizzles for 0 — try it twice!\nhero.attack(enemy);\nwizard.attack(enemy);\n',
    validate(p) {
      const mg = p.classes.find((c) => c.name === 'Mage')
