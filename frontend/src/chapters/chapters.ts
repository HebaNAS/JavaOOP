...
export const CHAPTERS: Chapter[] = [
  ...
  // ─── CHAPTER 4: Methods ───
  {
    ...
    hints: [
      'Methods are the verbs of a class. The game runs WHAT YOU WRITE inside them. attack() must subtract from target.health for the enemy to take damage; defend() must set this.shielded = true for the shield to activate.',
      'attack(Warrior target): "target.health = target.health - this.attackPower;" — the game reads target.health before/after your method, and the difference IS the damage. defend(): "this.shielded = true;" — the game reads that flag to know the shield went up.',
      'If you leave attack() empty, you swing but deal zero damage. If you leave defend() empty, no shield. Your code is what happens — write it carefully.',
    ],
    starter: 'class Warrior {\n    String name;\n    int health;\n    int attackPower;\n    boolean shielded;   // set true in defend() to activate the shield\n    int x;   // column on the map\n    int y;   // row on the map\n\n    Warrior(String name, int health, int attackPower) {\n        this.name = name;\n        this.health = health;\n        this.attackPower = attackPower;\n    }\n\n    // attack: reduce the target\'s health by this warrior\'s power\n    public void attack(Warrior target) {\n        target.health = target.health - this.attackPower;\n    }\n\n    // defend: raise the shield (game halves incoming damage when shielded == true)\n    public void defend() {\n        this.shielded = true;\n    }\n\n    // Movement — WASD / arrow keys bind to these at runtime\n    public void moveUp()    { this.y -= 1; }\n    public void moveDown()  { this.y += 1; }\n    public void moveLeft()  { this.x -= 1; }\n    public void moveRight() { this.x += 1; }\n}\n\nWarrior hero  = new Warrior("Aldric", 100, 25);\nWarrior enemy = new Warrior("Grok",    80, 20);\n\n// Click an enemy to target it (red ring), then SPACE = attack, Q = defend.\nhero.attack(enemy);\nhero.defend();\n',
    validate(p) {
      const c = p.classes.find((c) => c.name === 'Warrior')
      if (!c) return { pass: false, msg: 'Keep your Warrior class!' }
  ...
  // ─── CHAPTER 6: Inheritance ───
  {
    ...
    starter: 'class Warrior {\n    String name;\n    int health;\n    int attackPower;\n    boolean shielded;\n\n    Warrior(String name, int health, int attackPower) {\n        this.name = name;\n        this.health = health;\n        this.attackPower = attackPower;\n    }\n\n    public void attack(Warrior target) {\n        target.health = target.health - this.attackPower;\n    }\n    public void defend() { this.shielded = true; }\n}\n\n// Create: class Mage extends Warrior\n//   Include: int mana, castSpell(Warrior t) { target.health -= this.attackPower; }\n\nWarrior hero = new Warrior("Aldric", 100, 25);\n// Create a Mage object too!\n',
    validate(p) {
